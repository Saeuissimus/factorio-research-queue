...
function prompt_overwrite_research(player, research_name)
    if player.force.current_research ~= research_name then
        if player.gui.center.Q_exp then player.gui.center.Q_exp.destroy() end
        local warning = player.gui.center.add{type = "frame", name = "warning", style = "frame"}
        warning.add{type = "frame", name = "warning-icon", style = "rq-exp-warning-icon"}
        local text = warning.add{type = "flow", name = "rq-exp-warning-text", style = "vertical_flow", direction = "vertical"}
        local caption = {"rq-exp-gui.tech-prompt-overwrite", math.ceil(player.force.research_progress * 1000) / 10}
        text.add{type = "label", name = "rq-exp-warning-text-content", caption = caption, style = "description_label"}
        local buttons = text.add{type = "flow", name = "rq-exp-warning-buttons", style = "horizontal_flow", direction = "horizontal"}
        buttons.add{type = "button", name = "rq-exp-overwrite-yes", style = "button", caption = {"rq-exp-gui.accept"}}
        buttons.add{type = "button", name = "rq-exp-overwrite-no", style = "button", caption = {"rq-exp-gui.cancel"}}
    end
end
...
function update_queue_force(force, partial_update, players)
    ...
    for _, player in pairs(players) do
        if player.gui.center.Q_exp then
            last_row = math.max(last_row, player.mod_settings["research-queue-experimental-rows-count"].value + global.offset_queue[player.index])
...
function update_queue_player(player, partial_update, time_estimation)
    ...
    if time_estimation == nil then
        local last_row = player.mod_settings["research-queue-experimental-rows-count"].value + global.offset_queue[player.index]
...
function draw_queue_frame(player, partial_update)
    ...
    if not partial_update then
        player.opened = player.gui.center.Q_exp
        if not player.gui.center.Q_exp.current_q then
            player.gui.center.Q_exp.add{type = "frame", name = "current_q", caption = {"rq-exp-gui.current-queue"}, style = "technology_preview_frame"}
        end

        local list = nil
        if not player.gui.center.Q_exp.current_q.list then
            list = player.gui.center.Q_exp.current_q.add{type = "flow", name = "list", style = "rq-exp-flow-vertical", direction = "vertical"}
        else
            list = player.gui.center.Q_exp.current_q.list
            list.clear()
        end

        if #research_queue == 0 then
            list.add{type = "label", name = "empty", caption = {"rq-exp-gui.empty-queue"}}
        else
            list.add{type = "button", name = "rq-expscrollqueueup", style = "rq-exp-up-button", enabled = start > 1}
            local item_prototypes = game.item_prototypes
            for i = start, last do
                local tech_name = research_queue[i]
                descriptions[i] = draw_queued_technology(list, tech_name, force.technologies[tech_name], item_prototypes, player.mod_settings["research-queue-experimental-queued-tech-description-width"].value)
            end
            list.add{type = "button", name = "rq-expscrollqueuedown", style = "rq-exp-down-button", enabled = #research_queue > last_row}
        end
    else
        -- Construct the descriptions list to update them easily
        local list = player.gui.center.Q_exp.current_q.list
        for i = start, last do
            local tech_name = research_queue[i]
            local tech_description = list["rq-exp-frame-" .. tech_name].rqtechdescription
...
function draw_queued_technology(drawn_list, tech_name, technology, item_prototypes, description_width)
    ...
    local frame = drawn_list.add{type = "frame", name = "rq-exp-frame-" .. tech_name, style = "rq-exp-frame"}

    frame.add{type = "sprite-button", name = "rq-exp-icon" .. tech_name, style = "rq-exp-dummy-button",
              sprite = "technology/" .. tech_name, ignored_by_interaction = true}

    -- adds a description frame in the frame
    local description = frame.add{type = "flow", name = "rq-exptechdescription", style = "rq-exp-flow-vertical", direction = "vertical"}
    description.style.minimal_width = description_width
    -- places the name of the technology
    description.add{type = "label", name = "rq-exp" .. tech_name .. "name", caption = technology.localised_name, style = "description_label"}
    -- add the ingredients and time
    local ingredients = description.add{type = "table", name = "rq-exp" .. tech_name .. "ingredients", style = "table", column_count = 10}
    ingredients.add{type = "frame", name = "rq-exptime", caption = (technology.research_unit_energy / 60), style = "rq-exp-clock"}
    for _, item in pairs(technology.research_unit_ingredients) do
        ...
        local button = ingredients.add{type = "sprite-button", name = "rq-exp-ingredient" .. item.name, tooltip = item_localised_name,
                                       sprite = "item/" .. item.name, style = "rq-exp-ingredient-sprite"}
        button.add{type = "label", name = "rq-exp-label" .. item.name, style = "rq-exp-small-label",
                   caption = item.amount, ignored_by_interaction = true}
    end
    ingredients.add{type = "label", name = "rq-expresearch_unit_count", caption = "X " .. technology.research_unit_count, style = "label"}


    --adds the up/cancel/down buttons
    local buttons = frame.add{type = "table", name = "rq-exp" .. tech_name .. "buttons", style = "slot_table", column_count = 1}
    buttons.add{type = "button", name = "rq-expupbutton" .. tech_name, style = "rq-exp-up-button"}
    buttons.add{type = "button", name = "rq-expcancelbutton" .. tech_name, style = "rq-exp-cancel-button"}
    buttons.add{type = "button", name = "rq-expdownbutton" .. tech_name, style = "rq-exp-down-button"}
    return description
end
...
function update_estimated_time(tech_description, time_estimation)
    ...
    if time_estimation and time_estimation ~= math.huge then
        caption = {"rq-exp-gui.finite-eta", util.formattime(time_estimation)}
    else
        caption = {"rq-exp-gui.infinite-eta"}
    end

    if tech_description.rqtechtime then
        tech_description.rqtechtime.caption = caption
    else
        tech_description.add{type = "label", name = "rq-exptechtime", caption = caption, style = "label"}
    end
end