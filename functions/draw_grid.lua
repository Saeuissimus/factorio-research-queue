...
function options(player)
    local caption = player.gui.center.Q_exp.add2q.add{type = "label", name = "options_caption", caption = {"rq-exp-gui.options"}}
    caption.style.minimal_width = player.mod_settings["research-queue-experimental-table-width"].value * 68
    local columns = player.mod_settings["research-queue-experimental-table-width"].value
    ...
    local options = player.gui.center.Q_exp.add2q.add{type = "table", name = "options", style = "rq-exp-table2", column_count = columns}
    player.gui.center.Q_exp.add2q.add{type = "textfield", name = "rq-exp-text-filter", text = global.text_filter or "", tooltip = {"rq-exp-gui.prefix-text-search"}}

    options.add{type = "button", name = "rq-expextend-button", style = global.showExtended[player.index] and "rq-exp-compact-button" or "rq-exp-extend-button"}

    local text_view_style, text_view_tooltip = nil, nil
    if global.showIcon[player.index] then
        text_view_style = "rq-exp-tool-inactive-filter"
        text_view_tooltip = {"rq-exp-gui.use-tech-labels"}
    else
        text_view_style = "rq-exp-tool-selected-filter"
        text_view_tooltip = {"rq-exp-gui.use-tech-icons"}
    end
    options.add{type = "sprite-button", name = "rq-exptext", sprite = "rq-exp-text-view-icon",
    ...
    if global.showResearched[player.index] then
        completed_research_style = "rq-exp-tool-selected-filter"
        completed_research_tooltip = {"rq-exp-gui.stop-showing-researched-techs"}
    else
        completed_research_style = "rq-exp-tool-inactive-filter"
        completed_research_tooltip = {"rq-exp-gui.show-researched-techs"}
    end
    options.add{type = "sprite-button", name = "rq-expscience", sprite = "rq-exp-completed-research-icon",
    ...
    for name, science in pairs(global.science_packs) do
        if global.showExtended[player.index] or not (global.bobsmodules[name] or global.bobsaliens[name]) then
            ...
            if science[player.index] then
                filter_style = "rq-exp-tool-selected-filter"
                tooltip = {"rq-exp-gui.exclude_science_pack", item_prototypes[name].localised_name}
            else
                filter_style = "rq-exp-tool-inactive-filter"
                tooltip = {"rq-exp-gui.stop_science_pack_exclusion", item_prototypes[name].localised_name}
            end
            options.add{type = "sprite-button", name = "rq-exp-science" .. name,
...
function technologies(player, queued_techs)
    ...
    local caption = player.gui.center.Q_exp.add2q.add{type = "label", name = "add2q_caption", caption = {"rq-exp-gui.technology-list"}}
    caption.style.minimal_width = player.mod_settings["research-queue-experimental-table-width"].value * 68
    --create a smaller table if text is displayed.
    local column_count = nil
    local width_hack = nil
    if global.showIcon[player.index] then
        column_count = player.mod_settings["research-queue-experimental-table-width"].value
        width_hack = 34
    else
        column_count = math.floor(player.mod_settings["research-queue-experimental-table-width"].value / 3)
        width_hack = 3 * 34
    end
    local rq_table = player.gui.center.Q_exp.add2q.add{type = "table", name = "add2q_table", style = "rq-exp-table1", column_count = column_count}
    rq_table.style.width = player.mod_settings["research-queue-experimental-table-width"].value * 68 + width_hack
    local count = 0
          ...
          (global.showExtended[player.index] or not tech.upgrade or not any(tech.prerequisites, "upgrade") or
           any(tech.prerequisites, "researched") or matches(tech.prerequisites, "name", queued_techs)) then
            ...
            (
                ...
                local background = tech.researched and "rq-exp-done-" or queued_techs[tech.name] and "rq-exp-inq-" or "rq-exp-available-"
                count = count + 1
                if global.showIcon[player.index] then
                    -- Build GUI objects for icon view
                    background = background .. "frame"
                    local row = math.ceil(count / player.mod_settings["research-queue-experimental-table-width"].value)
                    if global.offset_tech[player.index] < row and row <= (player.mod_settings["research-queue-experimental-table-height"].value + global.offset_tech[player.index]) then
                        local bg_frame = rq_table.add{type = "frame", name = "rq-exp" .. name .. "background_frame", style = background}
                        --technology icon
                        local tech_icon = bg_frame.add{type = "sprite-button", name = "rq-exp-add" .. name,
                                                       sprite = "technology/" .. name, style = "rq-exp-button", tooltip = tech.localised_name}
                        ...
                        tech_icon.add{type = "label", name = name .. "label", style = "rq-exp-label", caption = caption,
                                      ignored_by_interaction = true, enabled = caption and string.len(caption) > 0}
                    elseif row > player.mod_settings["research-queue-experimental-table-height"].value + global.offset_tech[player.index] then
                    ...
                    local row = math.ceil(count / math.floor(player.mod_settings["research-queue-experimental-table-width"].value / 3))
                    if global.offset_tech[player.index] < row and row <= ((player.mod_settings["research-queue-experimental-table-height"].value*2) + global.offset_tech[player.index]) then
                        local frame = rq_table.add{type = "frame", name = "rq-exptextframe" .. name, style = "outer_frame"}

                        --technology icon
                        frame.add{type = "sprite-button", name = name .. "icon", style = "rq-exp-small-dummy-button", tooltip = tech.localised_name,
                                  sprite = "technology/" .. name, ignored_by_interaction = true}

                        frame.add{type = "button", name = "rq-exp-add" .. name, caption = tech.localised_name, style = background, tooltip = tech.localised_name}
                    elseif row > (player.mod_settings["research-queue-experimental-table-height"].value * 2) + global.offset_tech[player.index] then
                        should_draw_down_button = true
                        break
                    end
                end
    ...
    player.gui.center.Q_exp.add2q.add{type = "button", name = "rq-expscrolltechup", style = "rq-exp-up-button", enabled = global.offset_tech[player.index] > 0}
    player.gui.center.Q_exp.add2q.add{type = "button", name = "rq-expscrolltechdown", style = "rq-exp-down-button", enabled = should_draw_down_button}
end
...
function draw_grid_player(player, queued_techs)
    if player.gui.center.Q_exp then
        if player.gui.center.Q_exp.add2q then player.gui.center.Q_exp.add2q.destroy() end
        player.gui.center.Q_exp.add{type = "frame", name = "add2q", caption = {"rq-exp-gui.add-to-queue"}, style = "technology_preview_frame", direction = "vertical"}
        options(player)
