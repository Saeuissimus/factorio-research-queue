...
script.on_configuration_changed(function(event)
    if event.mod_changes ~= nil and event.mod_changes["research-queue-experimental"] ~= nil then
        init()
...
script.on_event(defines.events.on_gui_closed, function(event)
    ...
    if gui_type == defines.gui_type.custom and element and element.name == "Q_exp" then
        local player = game.players[event.player_index]
        player.gui.center.Q_exp.destroy()
        player.play_sound{path = "utility/gui_click"}
    end
end)

script.on_event(defines.events.on_gui_text_changed, function(event)
    if event.element.name == "rq-exp-text-filter" then
        local player = game.players[event.player_index]

        global.text_filter = event.element.text
        global.offset_tech[player.index] = 0
        draw_grid_player(player)
        player.gui.center.Q_exp.add2q["rq-exp-text-filter"].focus()
    end
end)
...
script.on_event(defines.events.on_gui_checked_state_changed, function(event)
    ...
    if event.element.name == "rq-exptext" then
    ...
    elseif event.element.name == "rq-exp-bobsmodules" then
    ...
    elseif event.element.name == "rq-exp-bobsaliens" then
...
script.on_event(defines.events.on_gui_click, function(event)
    ...
    if event.element.name == "rq-exp-text-filter" and event.button == defines.mouse_button_type.right then
        global.text_filter = ""
        global.offset_tech[player.index] = 0
        draw_grid_player(player)
    elseif event.element.name == "research_Q_exp" then
        toggle_gui_window(player)
    elseif string.find(event.element.name, "rq-exp-add", 1, true) == 1 then
        local tech = string.gsub(event.element.name, "rq%-exp%-add", "")
    ...
    elseif string.find(event.element.name, "rq-expcancelbutton", 1, true) == 1 then
        local tech = string.gsub(event.element.name, "rq%-expcancelbutton", "")
    ...
    elseif string.find(event.element.name, "rq-expupbutton", 1, true) == 1 then
        local tech = string.gsub(event.element.name, "rq%-expupbutton", "")
    ...
    elseif string.find(event.element.name, "rq-expdownbutton", 1, true) == 1 then
        local tech = string.gsub(event.element.name, "rq%-expdownbutton", "")
        if event.button ~= defines.mouse_button_type.left or event.control then
    ...
    elseif string.find(event.element.name, "rq-exp-science", 1, true) == 1 then
        global.offset_tech[player.index] = 0
        local tool = string.gsub(event.element.name, "rq%-exp%-science", "")
    ...
    elseif event.element.name == "rq-expscrolltechup" then
        global.offset_tech[player.index] = global.offset_tech[player.index] - 1
        draw_grid_player(player)

    elseif event.element.name == "rq-expscrolltechdown" then
        global.offset_tech[player.index] = global.offset_tech[player.index] + 1
        draw_grid_player(player)

    elseif event.element.name == "rq-exp-overwrite-yes" then
        force.current_research = global.researchQ[force.name][1]
        if player.gui.center.warning then player.gui.center.warning.destroy() end
        if not player.gui.center.Q_exp then player.gui.center.add{type = "flow", name = "Q_exp", style = "rq-exp-flow"} end
        update_queue_force(force)
        draw_grid_force(force)

    elseif event.element.name == "rq-exp-overwrite-no" then
        if player.gui.center.warning then player.gui.center.warning.destroy() end
        if not player.gui.center.Q_exp then player.gui.center.add{type = "flow", name = "Q_exp", style = "rq-exp-flow"} end
        update_queue_player(player)
        draw_grid_player(player)
...
script.on_event(defines.events.on_research_finished, function(event)
    ...
    for _, player in pairs(event.research.force.players) do
        player.print{"rq-exp-notices.research-finished", event.research.localised_name}
    end
...
script.on_event(defines.events.on_player_left_game, function(event)
    local Q_gui = game.players[event.player_index].gui.center.Q_exp
    if Q_gui then
        Q_gui.destroy()
    end
end)
...
script.on_event("open-research-queue-experimental", function(event)
